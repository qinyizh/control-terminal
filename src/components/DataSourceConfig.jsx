...
const DataSourceConfig = () => {
  ...
  const handleStartClient = async () => {
    try {
      const launchCommand = {
        LaunchGame: {
          UeType: 'Client',         // 类型：Server 或 Client
          PlayerName: 'Alice',      // 玩家名称
          CharacterType: 1          // 角色类型（数字）
        }
      };
      // 发送指令
     // sendMessage(launchCommand);
    } catch (error) {
  ...
  const checkTimelineStatus = async () => {
    try {
      const response = await fetch(`${API_BASE_URL}/timeline-status`);
  ...
  const handleLaunchGame = () => {
    console.log(Object.values(groups[0]['configs']));
    const params = []
